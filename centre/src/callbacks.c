...
/* gtk_calendar retourne mois = 0 à 11 → on doit ajouter 1 */
mois1 += 1;
mois2 += 1;
...
/* --- AJOUT : calcul pourcentages H/F --- */
int nb_h, nb_f;
calcul_pourcentage_hf(&nb_h, &nb_f);

int total = nb_h + nb_f;
float pour_h = (total > 0) ? (nb_h * 100.0 / total) : 0;
float pour_f = (total > 0) ? (nb_f * 100.0 / total) : 0;
...
char affichage2[500];
strcpy(affichage2, affichage);      // Copie base
strcat(affichage2, texte_final);    // Ajoute stats

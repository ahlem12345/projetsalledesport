...
{
    printf(" on_bouton_supprimer_clicked\n");
...
void on_fenetre_suppression_show(GtkWidget *widget, gpointer user_data) {
    printf(" Fenêtre suppression affichée\n");
                                        ...
                                        gpointer         user_data)
{
    ...
    GtkWidget *entrystat = lookup_widget(window, "entrystat");
    GtkWidget *resstat = lookup_widget(window, "resstat");

    int id = atoi(gtk_entry_get_text(GTK_ENTRY(entrystat)));

    if (id <= 0) {
        gtk_label_set_text(GTK_LABEL(resstat), "❌ ID invalide");
        return;
    }

    char *json = appel_api_stat(id);

    if (strstr(json, "error")) {
        gtk_label_set_text(GTK_LABEL(resstat), "❌ Coach non trouvé");
    } else {
        gtk_label_set_text(GTK_LABEL(resstat), json);
    }
